...
# --------------------------------------------------------------------------------
# initialize xbox controller using pygame
import pygame
pygame.init()
...
try:
    ...
    print("no controller detected:", e)
    exit()
...
def main(argv):
  ...
  # jax.jit compiles these functions into optimized code, speeding up repeated calls in the simulation loop.
  jit_reset = jax.jit(eval_env.reset)
  jit_step = jax.jit(eval_env.step)

  # --------------------------------------------------------------------------------
  # jax requires explicit randomness management with random keys.
  # this ensures that every random operation (like resetting the environment) is reproducible.
  ...
  # the environment should have an attribute 'mj_model' containing the mujoco model.
  if hasattr(eval_env, "mj_model"):
    print("creating mujoco simulation from the 'mj_model' attribute.")
    model = eval_env.mj_model
    data = mujoco.MjData(model)  # create a Mujoco data object to hold simulation state (e.g., joint positions)
  else:
    raise AttributeError("the evaluation environment does not have a 'mj_model' attribute for live visualization.")

  # disable internal random command updates so that our manual controller input isn't overridden.
  ...
  # This function updates the environment's observation by inserting the new command (e.g., our control inputs)
  # into the state's info, then re-computes the observation so that the policy sees the updated command.
  def update_observation_with_command(env_state, command):
    """update the state observation with the new command."""
    new_info = dict(env_state.info)  # copy current info to avoid modifying the original directly
    new_info["command"] = command      # update the command in the copied info
  ...
  # --------------------------------------------------------------------------------
  # initialize pygame controller input
  # we use pygame to read Xbox controller inputs (left stick and triggers)
  import pygame  # already imported above; ensure this is only called once
  pygame.init()
  pygame.joystick.init()
  try:
      # assume the first connected controller is the one we use.
      controller = pygame.joystick.Joystick(0)
      controller.init()
      print("xbox controller connected:", controller.get_name())
  except Exception as e:
      print("no xbox controller detected:", e)
      exit()

  # print instructions for using the controller (only these controls are used)
  ...
  # launch the mujoco viewer in passive mode (no keyboard callback needed now)
  with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    try:
        ...
        while viewer.is_running():
            # --------------------------------------------------------------------------------
            # poll the xbox controller using pygame
            pygame.event.pump()  # update the controller state
            
            # read the left analog stick values:
            # axis 0 for horizontal (left/right), axis 1 for vertical (up/down)
            x_axis = controller.get_axis(0)  # left stick horizontal: -1 (left) to 1 (right)
            y_axis = controller.get_axis(1)  # left stick vertical: typically -1 (up/forward) to 1 (down/backward)
            # here, we map the left stick to motion:
            # forward/backward command (forward is obtained by inverting y_axis)
            forward_vel = -y_axis  
            # lateral (sideways) command is taken directly from x_axis and inverted to map correct
            lateral_vel = -x_axis

            # read trigger values:
            # updated mappings:  axis 4 is left trigger, axis 5 is right trigger.
            left_trigger_value = controller.get_axis(4)
            right_trigger_value = controller.get_axis(5)
            # compute turning (yaw) command:
            # if the right trigger is pressed more than the left, this results in a positive value (yaw right).
            # if the left trigger is pressed more than the right, the result is negative (yaw left).
            ...
            # update the environment state's observation with this new command so that the policy sees it
            state = update_observation_with_command(state, command)

            # split rng for policy inference so randomness is managed correctly
            act_rng, rng = jax.random.split(rng)
            # run the inference function (which has been jit-compiled) to compute the control actions based on the current observation
            motor_action, _ = jit_inference_fn(state.obs, act_rng)

            # step the environment with the computed action, advancing the simulation state
            state = jit_step(state, motor_action)

            # synchronize the mujoco simulation's control signals with the brax state
            data.ctrl[:] = np.array(state.data.ctrl)
            # take one simulation step in mujoco
            mujoco.mj_step(model, data)
            # update mujoco's qpos and qvel (joint positions/velocities) to ensure the viewer accurately reflects the state
            ...
            # refresh the viewer display so you see the updated simulation
            viewer.sync()
            time.sleep(0.01)  # brief pause to control loop speed and reduce CPU load
